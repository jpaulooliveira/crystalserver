...
int NpcFunctions::luaNpcSellItem(lua_State* L) {
	...
	if (!g_game().validRemoveMoney(player, totalCost, 0, true)) {
		Lua::pushBoolean(L, false);
		player->sendCancelMessage(RETURNVALUE_NOTENOUGHROOM);
		g_logger().error("[NpcFunctions::luaNpcSellItem (validRemoveMoney)] - Player {} possible tried to abuse a bug fixed by jp {} on shop for npc {}", player->getName(), itemId, npc->getName());
		return 1;
	}
